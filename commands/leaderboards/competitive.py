...
class CxCCompetitiveLeaderboards(commands.Cog):
    def __init__(self, bot):
        ...
        self.current_stat_id = "30"
        self.current_period = "total"

        self.stat_info = {
            "total": {
                "32": {"name": "Vitórias", "stat_key": "competitive_wins", "emoji": "<a:wins:1348790119070564402>", "endpoint_suffix": "", "data_key": "accountStats"},
                "33": {"name": "Abates", "stat_key": "competitive_kills", "emoji": "<a:abates:1348799859603406979>", "endpoint_suffix": "", "data_key": "accountStats"},
                "34": {"name": "Mortes", "stat_key": "competitive_deaths", "emoji": "<a:skull:1348799160979030096>", "endpoint_suffix": "", "data_key": "accountStats"},
                "30": {"name": "XP", "stat_key": "competitive_exp", "emoji": "<a:xp:1348792513300795504>", "endpoint_suffix": "", "data_key": "accountStats"},
            },
            "monthly": {
                "32": {"name": "Vitórias", "stat_key": "competitive_wins", "emoji": "<a:wins:1348790119070564402>", "endpoint_suffix": "/monthly", "data_key": "accountStatsMonthly"},
                "33": {"name": "Abates", "stat_key": "competitive_kills", "emoji": "<a:abates:1348799859603406979>", "endpoint_suffix": "/monthly", "data_key": "accountStatsMonthly"},
                "34": {"name": "Mortes", "stat_key": "competitive_deaths", "emoji": "<a:skull:1348799160979030096>", "endpoint_suffix": "/monthly", "data_key": "accountStatsMonthly"},
                "30": {"name": "XP", "stat_key": "competitive_exp", "emoji": "<a:xp:1348792513300795504>", "endpoint_suffix": "/monthly", "data_key": "accountStatsMonthly"},
            },
            "weekly": {
                "32": {"name": "Vitórias", "stat_key": "competitive_wins", "emoji": "<a:wins:1348790119070564402>", "endpoint_suffix": "/weekly", "data_key": "accountStatsWeekly"},
                "33": {"name": "Abates", "stat_key": "competitive_kills", "emoji": "<a:abates:1348799859603406979>", "endpoint_suffix": "/weekly", "data_key": "accountStatsWeekly"},
                "34": {"name": "Mortes", "stat_key": "competitive_deaths", "emoji": "<a:skull:1348799160979030096>", "endpoint_suffix": "/weekly", "data_key": "accountStatsWeekly"},
                "30": {"name": "XP", "stat_key": "competitive_exp", "emoji": "<a:xp:1348792513300795504>", "endpoint_suffix": "/weekly", "data_key": "accountStatsWeekly"},
            },
        }

    cxc = app_commands.Group(name="cxc", description="Exibe as classificações do CxC Competitive.")
    ...
    async def fetch_leaderboard(self, stat_id: str, period: str):
        ...
        try:
            ...
            logging.error(f"Erro ao buscar a classificação de CxC {period} para statId {stat_id}: {e}")
            self.leaderboard_data[period] = None
    ...
    async def create_leaderboard_page(self, start_index: int, is_ephemeral: bool):
        ...
        if leaderboard_data_for_period is None:
            embed = discord.Embed(
                title="Erro na Classificação de CxC",
                description="Dados do placar de CxC não estão disponíveis ou houve um erro na comunicação com a API. Por favor, tente novamente mais tarde.",
        ...
        if not leaderboard_data_for_period:
            ...
            embed = discord.Embed(
                title=f"Classificação CxC Competitive ({period_text}) - {current_stat_name}",
        ...
        if page_data:
            ...
            embed.description = "Não há jogadores nesta página ou no placar de CxC Competitive."
        ...
        class LeaderboardView(discord.ui.View):
            def __init__(self, parent_cog, initial_start_index, original_user_id, is_ephemeral_view, original_interaction_for_edit):
                ...
                self.add_item(
                    SearchPlayerButton(self.parent_cog, self.original_user_id, self.is_ephemeral_view, self.original_interaction_for_edit)
                )
            ...
            def create_page_select(self):
                ...
                select = discord.ui.Select(
                    ...
                    options=options,
                    custom_id="page_select_cxc", # ID customizado para este select
        ...
        class StatButton(discord.ui.Button):
            def __init__(self, parent_cog, stat_id: str, label: str, emoji: str, original_user_id: int, is_ephemeral_button: bool):
                super().__init__(
                    ...
                    emoji=emoji,
                    custom_id=f"stat_button_cxc_{stat_id}", # ID customizado único para cada botão de estatística
                )
                self.parent_cog = parent_cog
        ...
        class SearchPlayerButton(discord.ui.Button):
            def __init__(self, parent_cog, original_user_id, is_ephemeral_button, original_interaction_for_edit):
                super().__init__(
                    label="Pesquisar Jogador",
                    style=discord.ButtonStyle.secondary,
                    custom_id="search_player_cxc", # ID customizado para este botão
                )
                self.parent_cog = parent_cog
                self.original_user_id = original_user_id
                self.is_ephemeral_button = is_ephemeral_button
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                class NickSearchModal(discord.ui.Modal, title="Pesquisar Jogador CxC Competitive"):
                    ...
                    async def on_submit(self, interaction: discord.Interaction):
                        ...
                        if found_player:
                            ...
                            target_page_start_index = (
                                (position - 1) // self.parent_cog.page_size
                            ) * self.parent_cog.page_size
                            self.parent_cog.current_page_start_index = (
                                target_page_start_index
                            )

                            embed, view = (
                                await self.parent_cog.create_leaderboard_page(
        ...
        view = LeaderboardView(self, start_index, self.original_interaction_user_id, is_ephemeral, self.original_interaction_for_edit)
        return embed, view

    @cxc.command(name="total", description="Exibe a classificação total do CxC Competitive.")
    async def total(self, interaction: discord.Interaction):
        """Comando para exibir o placar total do CxC Competitive."""
        ...
        if self.leaderboard_data.get(self.current_period) is None:
            await interaction.followup.send(
                "Erro ao buscar a classificação total do CxC Competitive. Tente novamente mais tarde.",
                ephemeral=True,
            )
            return
    ...
    @cxc.command(name="mensal", description="Exibe a classificação mensal do CxC Competitive.")
    async def mensal(self, interaction: discord.Interaction):
        """Comando para exibir o placar mensal do CxC Competitive."""
        should_be_ephemeral = False
        allowed_channels_cog = self.bot.get_cog("AllowedChannels")
        ...
        if self.leaderboard_data.get(self.current_period) is None:
            await interaction.followup.send(
                "Erro ao buscar a classificação mensal do CxC Competitive. Tente novamente mais tarde.",
                ephemeral=True,
            )
            return
    ...
    @cxc.command(name="semanal", description="Exibe a classificação semanal do CxC Competitive.")
    async def semanal(self, interaction: discord.Interaction):
        """Comando para exibir o placar semanal do CxC Competitive."""
        should_be_ephemeral = False
        allowed_channels_cog = self.bot.get_cog("AllowedChannels")
        ...
        if self.leaderboard_data.get(self.current_period) is None:
            await interaction.followup.send(
                "Erro ao buscar a classificação semanal do CxC Competitive. Tente novamente mais tarde.",
                ephemeral=True,
            )
            return
...
async def setup(bot):
    ...
    await bot.add_cog(CxCCompetitiveLeaderboards(bot))