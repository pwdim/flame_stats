...
class FlameLeagueLeaderboards(commands.Cog):
    def __init__(self, bot):
        ...
        self.current_stat_id = "25"
        self.current_period = "total"

        self.stat_info = {
            "total": {
                "27": {"name": "Vitórias", "stat_key": "league_wins", "emoji": "<a:wins:1348790119070564402>", "endpoint_suffix": "", "data_key": "accountStats"},
                "28": {"name": "Abates", "stat_key": "league_kills", "emoji": "<a:abates:1348799859603406979>", "endpoint_suffix": "", "data_key": "accountStats"},
                "29": {"name": "Mortes", "stat_key": "league_deaths", "emoji": "<a:skull:1348799160979030096>", "endpoint_suffix": "", "data_key": "accountStats"},
                "25": {"name": "XP", "stat_key": "league_exp", "emoji": "<a:xp:1348792513300795504>", "endpoint_suffix": "", "data_key": "accountStats"},
            },
            "monthly": {
                "27": {"name": "Vitórias", "stat_key": "league_wins", "emoji": "<a:wins:1348790119070564402>", "endpoint_suffix": "/monthly", "data_key": "accountStatsMonthly"},
                "28": {"name": "Abates", "stat_key": "league_kills", "emoji": "<a:abates:1348799859603406979>", "endpoint_suffix": "/monthly", "data_key": "accountStatsMonthly"},
                "29": {"name": "Mortes", "stat_key": "league_deaths", "emoji": "<a:skull:1348799160979030096>", "endpoint_suffix": "/monthly", "data_key": "accountStatsMonthly"},
                "25": {"name": "XP", "stat_key": "league_exp", "emoji": "<a:xp:1348792513300795504>", "endpoint_suffix": "/monthly", "data_key": "accountStatsMonthly"},
            },
            "weekly": {
                "27": {"name": "Vitórias", "stat_key": "league_wins", "emoji": "<a:wins:1348790119070564402>", "endpoint_suffix": "/weekly", "data_key": "accountStatsWeekly"},
                "28": {"name": "Abates", "stat_key": "league_kills", "emoji": "<a:abates:1348799859603406979>", "endpoint_suffix": "/weekly", "data_key": "accountStatsWeekly"},
                "29": {"name": "Mortes", "stat_key": "league_deaths", "emoji": "<a:skull:1348799160979030096>", "endpoint_suffix": "/weekly", "data_key": "accountStatsWeekly"},
                "25": {"name": "XP", "stat_key": "league_exp", "emoji": "<a:xp:1348792513300795504>", "endpoint_suffix": "/weekly", "data_key": "accountStatsWeekly"},
            },
        }

    flameleague = app_commands.Group(name="flameleague", description="Exibe as classificações da FlameLeague.")
    ...
    async def fetch_leaderboard(self, stat_id: str, period: str):
        ...
        if not selected_stat_info:
            logging.error(f"Error: No stat info found for period '{period}' and statId '{stat_id}'")
        ...
        try:
            ...
            logging.error(f"Error fetching FlameLeague {period} leaderboard for statId {stat_id}: {e}")
            self.leaderboard_data[period] = None
    ...
    async def create_leaderboard_page(self, start_index: int, is_ephemeral: bool):
        ...
        if leaderboard_data_for_period is None:
            embed = discord.Embed(
                title="Erro na Classificação FlameLeague",
                description="Dados do placar da FlameLeague não estão disponíveis ou houve um erro na comunicação com a API. Por favor, tente novamente mais tarde.",
        ...
        if not leaderboard_data_for_period:
            ...
            embed = discord.Embed(
                title=f"Classificação FlameLeague ({period_text}) - {current_stat_name}",
        ...
        if page_data:
            ...
            embed.description = "Não há jogadores nesta página ou no placar de FlameLeague."
        ...
        class LeaderboardView(discord.ui.View):
            ...
            def create_page_select(self):
                ...
                select = discord.ui.Select(
                    ...
                    options=options,
                    custom_id="page_select_flameleague", # ID customizado para este select
        ...
        class StatButton(discord.ui.Button):
            def __init__(self, parent_cog, stat_id: str, label: str, emoji: str, original_user_id: int, is_ephemeral_button: bool):
                super().__init__(
                    ...
                    emoji=emoji,
                    custom_id=f"stat_button_flameleague_{stat_id}", # ID customizado único para cada botão de estatística
                )
                self.parent_cog = parent_cog
        ...
        class SearchPlayerButton(discord.ui.Button):
            def __init__(self, parent_cog, original_user_id, is_ephemeral_button, original_interaction_for_edit):
                super().__init__(
                    ...
                    custom_id="search_player_flameleague", # ID customizado para este botão
                )
                self.parent_cog = parent_cog
                self.original_user_id = original_user_id
                self.is_ephemeral_button = is_ephemeral_button
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                class NickSearchModal(discord.ui.Modal, title="Pesquisar Jogador FlameLeague"):
    ...
    @flameleague.command(name="total", description="Exibe a classificação total da FlameLeague.")
    async def total(self, interaction: discord.Interaction):
        """Comando para exibir o placar total da FlameLeague."""
        ...
        if self.leaderboard_data.get(self.current_period) is None:
            await interaction.followup.send(
                "Erro ao buscar a classificação total da FlameLeague. Tente novamente mais tarde.",
                ephemeral=True,
            )
            return
    ...
    @flameleague.command(name="mensal", description="Exibe a classificação mensal da FlameLeague.")
    async def mensal(self, interaction: discord.Interaction):
        """Comando para exibir o placar mensal da FlameLeague."""
        should_be_ephemeral = False
        allowed_channels_cog = self.bot.get_cog("AllowedChannels")
        ...
        if self.leaderboard_data.get(self.current_period) is None:
            await interaction.followup.send(
                "Erro ao buscar a classificação mensal da FlameLeague. Tente novamente mais tarde.",
                ephemeral=True,
            )
            return
    ...
    @flameleague.command(name="semanal", description="Exibe a classificação semanal da FlameLeague.")
    async def semanal(self, interaction: discord.Interaction):
        """Comando para exibir o placar semanal da FlameLeague."""
        should_be_ephemeral = False
        allowed_channels_cog = self.bot.get_cog("AllowedChannels")
        ...
        if self.leaderboard_data.get(self.current_period) is None:
            await interaction.followup.send(
                "Erro ao buscar a classificação semanal da FlameLeague. Tente novamente mais tarde.",
                ephemeral=True,
            )
            return
...
async def setup(bot):
    ...
    await bot.add_cog(FlameLeagueLeaderboards(bot))