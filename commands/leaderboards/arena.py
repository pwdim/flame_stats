...
class ArenaLeaderboards(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.leaderboard_data = {}  # Armazena os dados do placar para cada período
        self.page_size = 10         # Número de jogadores por página
        self.max_position = 100     # Limite máximo de posições a serem exibidas no placar
        self.current_page_start_index = 0 # Índice inicial da página atual
        self.original_interaction_user_id = None # ID do usuário que invocou o comando
        self.current_stat_id = "3"  # ID da estatística padrão (XP da Arena)
        self.current_period = "total" # Período padrão (total)

        # Dicionário contendo informações sobre as estatísticas disponíveis por período.
        # As cores dos botões são limitadas aos estilos predefinidos do Discord.
        # Emojis são usados para adicionar distinção visual.
        self.stat_info = {
            "total": {
                "1": {"name": "Abates", "stat_key": "pvp_arena_kills", "emoji": "<a:abates:1348799859603406979>", "endpoint_suffix": "", "data_key": "accountStats"},
                "2": {"name": "Mortes", "stat_key": "pvp_arena_deaths", "emoji": "<a:skull:1348799160979030096>", "endpoint_suffix": "", "data_key": "accountStats"},
                "3": {"name": "XP", "stat_key": "pvp_exp", "emoji": "<a:xp:1348792513300795504>", "endpoint_suffix": "", "data_key": "accountStats"},
                "6": {"name": "Melhor Streak", "stat_key": "pvp_arena_best_streak", "emoji": "<a:best_streak:1348793098175647764>", "endpoint_suffix": "", "data_key": "accountStats"},
            },
            "weekly": {
                "1": {"name": "Abates", "stat_key": "pvp_arena_kills", "emoji": "<a:abates:1348799859603406979>", "endpoint_suffix": "/weekly", "data_key": "accountStatsWeekly"},
                "2": {"name": "Mortes", "stat_key": "pvp_arena_deaths", "emoji": "<a:skull:1348799160979030096>", "endpoint_suffix": "/weekly", "data_key": "accountStatsWeekly"},
                "3": {"name": "XP", "stat_key": "pvp_exp", "emoji": "<a:xp:1348792513300795504>", "endpoint_suffix": "/weekly", "data_key": "accountStatsWeekly"},
                "6": {"name": "Melhor Streak", "stat_key": "pvp_arena_best_streak", "emoji": "<a:best_streak:1348793098175647764>", "endpoint_suffix": "/weekly", "data_key": "accountStatsWeekly"},
            },
            "monthly": {
                "1": {"name": "Abates", "stat_key": "pvp_arena_kills", "emoji": "<a:abates:1348799859603406979>", "endpoint_suffix": "/monthly", "data_key": "accountStatsMonthly"},
                "2": {"name": "Mortes", "stat_key": "pvp_arena_deaths", "emoji": "<a:skull:1348799160979030096>", "endpoint_suffix": "/monthly", "data_key": "accountStatsMonthly"},
                "3": {"name": "XP", "stat_key": "pvp_exp", "emoji": "<a:xp:1348792513300795504>", "endpoint_suffix": "/monthly", "data_key": "accountStatsMonthly"},
                "6": {"name": "Melhor Streak", "stat_key": "pvp_arena_best_streak", "emoji": "<a:best_streak:1348793098175647764>", "endpoint_suffix": "/monthly", "data_key": "accountStatsMonthly"},
            },
        }

    # Cria um grupo de comandos de aplicativo para "arena"
    arena = app_commands.Group(name="arena", description="Exibe as classificações da Arena do FlameMC.")
    ...
    async def fetch_leaderboard(self, stat_id: str, period: str):
        ...
        try:
            ...
            logging.error(f"Erro ao buscar o placar da Arena ({period}) para statId {stat_id}: {e}")
            self.leaderboard_data[period] = None
    ...
    async def create_leaderboard_page(self, start_index: int, is_ephemeral: bool):
        ...
        if leaderboard_data_for_period is None:
            embed = discord.Embed(
                title="Erro na Classificação da Arena",
                description="Dados do placar da Arena não estão disponíveis ou houve um erro na comunicação com a API. Por favor, tente novamente mais tarde.",
        ...
        if not leaderboard_data_for_period:
            ...
            embed = discord.Embed(
                title=f"Classificação da Arena ({period_text}) - {current_stat_name}",
        ...
        if page_data:
            ...
            embed.description = "Não há jogadores nesta página ou no placar da Arena."
        ...
        class LeaderboardView(discord.ui.View):
            ...
            def create_page_select(self):
                ...
                if not options: # Caso não haja páginas disponíveis
                    options.append(
                        discord.SelectOption(
                ...
                select = discord.ui.Select(
                    ...
                    options=options,
                    custom_id="page_select_arena", # ID customizado para este select
            ...
            async def page_select_callback(self, interaction: discord.Interaction):
                ...
                await interaction.response.defer() # Deferir a resposta
        ...
        class StatButton(discord.ui.Button):
            def __init__(self, parent_cog, stat_id: str, label: str, emoji: str, original_user_id: int, is_ephemeral_button: bool):
                super().__init__(
                    ...
                    emoji=emoji,
                    custom_id=f"stat_button_arena_{stat_id}", # ID customizado único para cada botão de estatística
                )
                self.parent_cog = parent_cog
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                self.parent_cog.current_page_start_index = 0 # Reinicia para a primeira página ao mudar a estatística
        ...
        class SearchPlayerButton(discord.ui.Button):
            def __init__(self, parent_cog, original_user_id, is_ephemeral_button):
                super().__init__(
                    ...
                    custom_id="search_player_arena", # ID customizado para este botão
                )
                self.parent_cog = parent_cog
                self.original_user_id = original_user_id
                self.is_ephemeral_button = is_ephemeral_button
            ...
            async def callback(self, interaction: discord.Interaction):
                ...
                class NickSearchModal(discord.ui.Modal, title="Pesquisar Jogador da Arena"):
                    ...
                    async def on_submit(self, interaction: discord.Interaction):
                        ...
                        await interaction.response.defer(ephemeral=self.is_ephemeral_modal)
                        leaderboard_data_for_period = self.parent_cog.leaderboard_data.get(self.parent_cog.current_period)
                        ...
                        if found_player:
                            ...
                            # Calcula o índice da página onde o jogador foi encontrado
                            ...
                            embed, view = await self.parent_cog.create_leaderboard_page(
                                target_page_start_index, self.is_ephemeral_modal
                            )
                            
                            await interaction.edit_original_response(embed=embed, view=view)
                            
                            await interaction.followup.send(
                                f"Encontrado **{search_nick}** na posição **#{position}**. Navegando para a página **{(target_page_start_index // self.parent_cog.page_size) + 1}**.",
                                ephemeral=True,
                            )
                        else:
                            await interaction.followup.send(
    ...
    @arena.command(name="total", description="Exibe a classificação total da Arena.")
    async def total(self, interaction: discord.Interaction):
        """Comando para exibir o placar total da Arena."""
        should_be_ephemeral = False
        # Verifica se o canal atual está na lista de canais permitidos (se o cog AllowedChannels estiver carregado)
        ...
        await interaction.response.defer(ephemeral=should_be_ephemeral) # Deferir a resposta

        self.original_interaction_user_id = interaction.user.id
        self.current_period = "total"
        self.current_stat_id = "3" # Define a estatística padrão para XP da Arena
        ...
        if self.leaderboard_data.get(self.current_period) is None:
            await interaction.followup.send(
                "Erro ao buscar a classificação total da Arena. Tente novamente mais tarde.",
                ephemeral=True,
            )
            return
    ...
    @arena.command(name="mensal", description="Exibe a classificação mensal da Arena.")
    async def mensal(self, interaction: discord.Interaction):
        """Comando para exibir o placar mensal da Arena."""
        should_be_ephemeral = False
        allowed_channels_cog = self.bot.get_cog("AllowedChannels")
        ...
        if self.leaderboard_data.get(self.current_period) is None:
            await interaction.followup.send(
                "Erro ao buscar a classificação mensal da Arena. Tente novamente mais tarde.",
                ephemeral=True,
            )
            return
    ...
    @arena.command(name="semanal", description="Exibe a classificação semanal da Arena.")
    async def semanal(self, interaction: discord.Interaction):
        """Comando para exibir o placar semanal da Arena."""
        should_be_ephemeral = False
        allowed_channels_cog = self.bot.get_cog("AllowedChannels")
        ...
        if self.leaderboard_data.get(self.current_period) is None:
            await interaction.followup.send(
                "Erro ao buscar a classificação semanal da Arena. Tente novamente mais tarde.",
                ephemeral=True,
            )
            return
...
async def setup(bot):
    ...
    await bot.add_cog(ArenaLeaderboards(bot))