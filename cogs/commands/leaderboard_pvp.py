...
class PVPLeaderboard(commands.Cog):
    ...
    async def fetch_leaderboard(self):
        url = "https://api.flamemc.com.br/leaderboards?statId=3&size=200"  # Assuming statId 19 is still for Competitive
        try:
    ...
    competitive_rank_colors = {  # Changed to competitive_rank_colors
    ...
    @app_commands.command(name="leader_pvp", description="Exibe a classificação do PvP.")
    async def competitive_leaderboard(self, interaction: discord.Interaction): # changed command name and function name
        ...
        await self.send_leaderboard_page(interaction, 0)

    async def send_leaderboard_page(self, interaction: discord.Interaction, start_index: int):
        ...
        embed = discord.Embed(title=" Classificação PvP", color=discord.Color.dark_green()) # changed embed title
        ...
        for player in page_data:
            ...
            if player_data and player_data.get("accountStats"):
                for stat in player_data["accountStats"]:
                    if stat["statsMap"]["name"] == "pvp_exp": #assuming hg_exp is still the correct stat
                        exp = stat["value"]
                        break
            ...
            embed_color = self.competitive_rank_colors.get(rank_name, discord.Colour.gold()) # changed rank colors
        ...
        async def prev_callback(interaction: discord.Interaction):
            await interaction.response.defer()
            await self.send_leaderboard_page(interaction, start_index - self.page_size)
...
async def setup(bot):
    await bot.add_cog(PVPLeaderboard(bot)) # changed cog nameimport discord
import requests
from discord.ext import commands
from discord import app_commands
...
class PVPLeaderboard(commands.Cog):
    ...
    async def fetch_leaderboard(self):
        url = "https://api.flamemc.com.br/leaderboards?statId=3&size=200"  # Assuming statId 3 is correct for PvP
        try:
            response = requests.get(url)
            response.raise_for_status()
            self.leaderboard_data = response.json()
    ...
    @app_commands.command(name="leader_pvp", description="Exibe a classificação do PvP.")
    async def competitive_leaderboard(self, interaction: discord.Interaction):
        await interaction.response.defer()
        await self.fetch_leaderboard()
    ...
    async def create_leaderboard_page(self, start_index: int):
        ...
        embed = discord.Embed(title=" Classificação PvP", color=discord.Color.dark_green())
        ...
        for player in page_data:
            ...
            rank = self.get_rank(exp, position)
            rank_name = rank.split(" ")[1] if " " in rank else rank
            embed_color = self.competitive_rank_colors.get(rank_name, discord.Colour.gold())
            embed.color = embed_color
            banned_text = "<:barrier:1348790166344695841> BANIDO <:barrier:1348790166344695841>" if banned else ""
            formatted_exp = "{:,}".format(exp).replace(",", ".")
            cdata = player["clan"]
...
async def setup(bot):
    await bot.add_cog(PVPLeaderboard(bot))