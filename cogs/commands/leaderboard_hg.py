...
class HGLeaderboard(commands.Cog):
    ...
    @app_commands.command(name="leader_hg", description="Exibe a classificação do HG.")
    async def hg_leaderboard(self, interaction: discord.Interaction):
    ...
    async def create_leaderboard_page(self, start_index: int):
        ...
        embed = discord.Embed(title=" Classificação HG", color=discord.Color.dark_green())
...
async def setup(bot):
    await bot.add_cog(HGLeaderboard(bot))