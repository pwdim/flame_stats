...
class CompetitiveLeaderboard(commands.Cog):
    ...
    async def fetch_leaderboard(self):
        url = "https://api.flamemc.com.br/leaderboards?statId=30&size=200"  # Assuming statId 30 is still for Competitive
        try:
    ...
    @app_commands.command(name="leader_cxc", description="Exibe a classificação do Competitive.")
    ...
    async def create_leaderboard_page(self, start_index: int):
        ...
        embed = discord.Embed(title=" Classificação CxC", color=discord.Color.dark_green()) # changed embed title
        ...
        for player in page_data:
            ...
            if player_data and player_data.get("accountStats"):
                for stat in player_data["accountStats"]:
                    if stat["statsMap"]["name"] == "competitive_exp": #assuming hg_exp is still the correct stat
                        exp = stat["value"]
                        break
...
async def setup(bot):
    await bot.add_cog(CompetitiveLeaderboard(bot)) # changed cog name